...
import com.sofn.service.sys.SSOLoginService;
...
public class LoginController extends BaseController {
    ...
    private final SysOrganizationService sysOrgService;
    private final RedisService           redisService;
    // 使用构造注入
    @Autowired(required=false)
    public LoginController(SSOLoginService ssoLoginService, SysOrganizationService sysOrgService, RedisService redisService) {this.ssoLoginService = ssoLoginService;
    ...
    @SystemControllerLog(description = "保存Cookie",operationType="修改")
    @ApiOperation(value = "保存Cookie")
    @RequestMapping(value = "/saveCookie")
    protected Object saveCookie(ModelMap modelMap,HttpServletRequest request, HttpServletResponse response,@ApiParam(required = false, value = "记住密码")
    @RequestParam(name = "remember", required = false) String remember,@ApiParam(required = false, value = "用户名")
    @RequestParam(name = "password", required = false) String password,@ApiParam(required = false, value = "密码")
...
//        String remember=request.getParameter("remember");
    ...
    public Object generatorCode(ModelMap modelMap, HttpServletRequest request,HttpServletResponse response,
        ...
        String code = (String) RedisUtil.get(uuid,ApiConstants.VALIDATE_CODE_LEN);
    ...
    @SystemControllerLog(description = "用户登录",operationType="登录")
...
//                            @RequestParam(name = "code", required = false) String code) {
    public Object login(ModelMap modelMap,@ApiParam(required = true, value = "密文")
        ...
        if (ValidateCodeUtil.checkCode(uuid, code)) {
            ...
            if(status != null)
                if("2".equals(status)){
                    throw new LoginException("登录失败，账号已注销！");
                }else if("3".equals(status)){
                    throw new LoginException("登录失败，注销申请中！");
                }else if("4".equals(status)){
                    throw new LoginException("登录失败，撤销申请中！");
     ...
     * @return the object
     */
    @SystemControllerLog(description = "用户登出",operationType="登出")
